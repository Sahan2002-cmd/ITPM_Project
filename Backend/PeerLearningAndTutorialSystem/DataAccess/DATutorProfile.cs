...
namespace PeerLearningAndTutorialSystem.DataAccess
{
    ...
    public class DATutorProfile : ITutorProfile
    {
        ...
        public Response CreateProfile(TutorProfileRequestApi request)
        {
            try
            {
                ...
                var profile = new TutorProfileModel
                {
                    ...
                    TeachingStyles    = request.TeachingStyles ?? new List<string>(),
        ...
        // ═══════════════════════════════════════════════════════════════════════
        // 006 – GET ALL PROFILES (Admin)
        // Returns every profile regardless of status, newest first.
        // ═══════════════════════════════════════════════════════════════════════
        public Response GetAllProfilesAdmin()
        {
            try
            {
                var profiles = _profiles.Find(_ => true)
        ...
        // ═══════════════════════════════════════════════════════════════════════
        // 007 – APPROVE PROFILE  (Admin)
        // Sets Status = "Active" and IsVerified = true.
        // Returns the tutor's UserId so the caller can send a notification.
        // ═══════════════════════════════════════════════════════════════════════
        public Response ApproveProfile(string id)
        {
            try
            {
                var existing = _profiles.Find(p => p.Id == id).FirstOrDefault();
                ...
                if (existing.Status == "Active")
                    return Response.Fail("This tutor profile is already active.");
                ...
                _profiles.UpdateOne(p => p.Id == id, update);
                return Response.Success(new { UserId = existing.UserId, FullName = existing.FullName },
                                        "Tutor profile approved and activated.");
            }
            catch (Exception ex) { return Response.Error(ex.Message); }
        }
