    ...
    <div className="flex h-full bg-slate-50 dark:bg-slate-900">
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-700/50">
          ...
          <div className="relative">
            ...
            <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Search..."
              className="w-full pl-9 pr-3 py-2 text-sm bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500/20" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            ...
            <button key={p.userId} onClick={() => { setSel(p); setMenuId(null); setEditId(null); }}
              className={`w-full flex items-center gap-3 p-4 border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-left ${sel?.userId === p.userId ? "bg-violet-50 dark:bg-violet-900/20 border-l-2 border-l-violet-600" : ""}`}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-800 dark:text-white truncate">{p.fullName}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{p.roleName}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-900">
        {sel ? (
          <>
            {/* header */}
            <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-5 py-3 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-800 dark:text-white">{sel.fullName}</p>
                  <p className="text-xs text-emerald-500 font-medium">Online</p>
                </div>
              </div>
              <button onClick={handleRefresh} disabled={refreshing} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-colors">
                      ...
                      {isEditing ? (
                        <div className="flex flex-col gap-1">
                          <textarea value={editText} onChange={e => setEditText(e.target.value)}
                            className="px-3 py-2 text-sm border border-violet-300 dark:border-violet-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500/30 resize-none"
                        ...
                        <>
                          ...
                          {isMe && (
                            <button onClick={() => setMenuId(menuId === mId ? null : mId)}
                              className="absolute -top-1 right-0 opacity-0 group-hover:opacity-100 transition-opacity p-1 bg-white dark:bg-slate-800 rounded-full shadow border border-slate-100 dark:border-slate-700">
                              <MoreVertical className="w-3 h-3 text-slate-500 dark:text-slate-400" />
                            </button>
                          )}
                          {menuId === mId && isMe && (
                            <div className="absolute top-6 right-0 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 py-1 z-10 min-w-[120px]">
                              {canE && (
                                <button onClick={() => startEdit(m)}
                                  className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-violet-50 dark:hover:bg-slate-700 transition-colors">
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1 relative">
                  <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={onKey}
                    ...
                    className="w-full px-4 py-2.5 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 rounded-2xl focus:outline-none focus:ring-2 focus:ring-violet-500/20 resize-none" />
                  {text.length > 0 && (
